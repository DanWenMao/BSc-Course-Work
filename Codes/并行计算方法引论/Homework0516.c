...
//  compute the coordinates of (0, 0) of the subdomain
    ...
    if ( myidx < (nx-1) % npx )  ix0 = myidx * ((nx-1)/npx + 1);
    else  ix0 = myidx * ((nx-1)/npx) + (nx-1) % npx;
    if ( myidy < (ny-1) % npy )  iy0 = myidy * ((ny-1)/npy + 1);
    else  iy0 = myidy * ((ny-1)/npy) + (ny-1) % npy;
...
//  begin iteration loop
    for (iter=1; iter<=itmax; iter++)
    {
        ...
        MPI_Sendrecv(&u[1][1], 1, type1, below, 222, &u[1][0], 1, type1, below, 222, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&u[1][nly-1], 1, type1, above, 222, &u[1][nly],   1, type1, above, 222, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&u[1][1], 1, type2, left,  222, &u[0][1], 1, type2, left,  222, MPI_COMM_WORLD, &status);
        MPI_Sendrecv(&u[nlx-1][1], 1, type2, right, 222, &u[nlx][1],   1, type2, right, 222, MPI_COMM_WORLD, &status);

        for (i=1; i<nlx; i++)
        for (j=1; j<nly; j++) {
            if ( ( (int)(ix0+i) + (int)(iy0+j) ) % 2 != 0) {
                u[i][j] = f[i][j] + dx * (u[i+1][j] + u[i-1][j])
                                   + dy * (u[i][j+1] + u[i][j-1]);
            }
        }
